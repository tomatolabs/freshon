...
var authenticate = function (req, res, next) {
    ...
    if(!utoken){
        ...
        var onUserSaved = function(err, user){
            ...
            setUserAuthenticated(next, req, res, uid, true);
        };
        ...
        if(!validatedUser){
            ...
            var onUserLoaded = function(err, doc){
                ...
                setUserAuthenticated(next, req, res, user.uid, user.state);
            };
...
var setUserAuthenticated = function(next, req, res, uid, state){
    var user = {
        id: uid,
        state: state,
    ...
    logger.debug('user [' + user.id + '] is authenticated requesting ' + req.originalUrl);
    next();
};
